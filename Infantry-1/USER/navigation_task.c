...
void Set_Initpoint_forNavigation(void)
{
	...
	{
		if(RC_CtrlData.rc.s1 == 1 && last_rc_s1 == 3 && Set_Navipoint_status == 0)//开启设置
		{
			Set_Navipoint_status = 1;
		}
		else if(RC_CtrlData.rc.s1 == 1 && last_rc_s1 == 3 && Set_Navipoint_status == 1)//关闭摩擦轮
		{
			Set_Navipoint_status = 0;
		}
		if(RC_CtrlData.rc.s1 == 2 && last_rc_s1 == 3 && Set_Navipoint_status == 1) //发射子弹
		{
			Set_Navipoint_cnt++;
