...
public:
    RoboControl(){
        ...
        pub_game_info=pnh->advertise<whurobot_msgs::GameInfo>("game_info",1);
        ...
        pub_imu_data=pnh->advertise<sensor_msgs::Imu>("imu_data",1);
    };
...
int main(int argc,char** argv)
{
    ...
    ros::Publisher pub_pose_plan=nh.advertise<geometry_msgs::Pose>("goal_plan",1);
    ...
    while (ros::ok())
    {
        ...
        if (abs(current_goal_x-enemy_pose_x)>1|| abs(current_goal_y-enemy_pose_y)>1){
            if(enemy_flag==1){
                goal_plan_msg.position.x=enemy_pose_x;
                goal_plan_msg.position.y=enemy_pose_y;
                goal_plan_msg.orientation=tf::createQuaternionMsgFromRollPitchYaw(0,0,0);
                pub_pose_plan.publish(goal_plan_msg);
