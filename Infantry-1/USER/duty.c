...
//抢中点段计时：
int grab_center_segmentcnt = 0, grab_center_segmentflag = 0;
extern int grab_centerpoint_TIMER ;
extern int grab_centerpoint_status;

extern uint16_t rfid_detect_cnt , rfid_detect_last_cnt;
uint16_t rfid_to_buff_cnt , rfid_to_buff_1s_cnt , get_the_attacking_buff_flag = 1;

int game_start_flag , game_start_cnt;
void TIM3_IRQHandler(void) //1000hz
{
	if(TIM_GetITStatus(TIM3 , TIM_IT_Update) != RESET)
	{
		...
		{
			uwb_selfcheck_cnt++;
			if(uwb_selfcheck_cnt >= 90000)
			{
				uwb_Selfcheck_flag = 2;
				uwb_selfcheck_cnt = 90000;
		...
		else if(uwb_Selfcheck_flag == 0) //没断电
		{
			uwb_selfcheck_cnt++;
			if(uwb_selfcheck_cnt >= 1000)
			{
				uwb_Selfcheck_flag = 2;
				uwb_selfcheck_cnt = 1000;
				Uwb_data.init_yaw_angle_uwb = Uwb_data.yaw_angle_uwb;
				Map_data.point_init.x       = Uwb_data.x_uwb;
				Map_data.point_init.y       = Uwb_data.y_uwb;
			}
		...
		//buff 5s检测
		if(rfid_detect_cnt > 0)
		{
			rfid_to_buff_cnt++;
			if(rfid_to_buff_cnt >= 500)
			{
				rfid_to_buff_cnt = 0;
				
				rfid_to_buff_1s_cnt++;
				if(rfid_to_buff_1s_cnt >= 11)
				{
					rfid_to_buff_1s_cnt = 0;
					get_the_attacking_buff_flag = 1;
				}
				
				if(get_the_attacking_buff_flag == 0)
				{
					if(f(rfid_detect_last_cnt - rfid_detect_cnt) < 1)
					{
						rfid_to_buff_1s_cnt = 0;
						rfid_detect_cnt = 0;
					}
				}
				rfid_detect_last_cnt = rfid_detect_cnt;
			}
		}
...
extern int chassis_turn_tail_stopgap_flag;
int chassis_turn_tail_stopgap_cnt = 1;
void TIM4_IRQHandler(void)
{
	if(TIM_GetITStatus(TIM4 , TIM_IT_Update) != RESET)
	{
		...
		{
			chassis_turn_tail_stopgap_cnt++;
			if(chassis_turn_tail_stopgap_cnt >= 300)
			{
				chassis_turn_tail_stopgap_cnt = 1;
				chassis_turn_tail_stopgap_flag = 0;
			}
		}
	  ...
	  if(rxbuf[ccnt + 15] == 0x7E)    //接收到包尾                  
		{
     	...
     	Top_Data.Top_Gimbal_Mode  =  rxbuf[ccnt + 2];
			...
			{
        yaw_angle_2D = Top_Data.Top_velocity_yaw;
			}
		...
		{	
		  ...
		  if(rx_message_DLC_flag == 1 && rx_message.DLC == 8)
			{		
				...
				Uwb_data.x_uwb           =  rx_message.Data[1] << 8 | rx_message.Data[0]; //800
				Uwb_data.y_uwb           =  rx_message.Data[3] << 8 | rx_message.Data[2]; //500
